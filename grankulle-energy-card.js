...
class GrankulleEnergyCard extends HTMLElement {
  ...
  // Swedish convention: prices under 1 SEK/kWh read easier as \u00f6re
  _fmtPrice(price) {
    if (price === null || price === undefined || isNaN(price)) return { text: '\u2013', unit: '' };
    if (price < 1) return { text: (price * 100).toFixed(1), unit: '\u00f6re/kWh' };
    return { text: price.toFixed(3), unit: 'SEK/kWh' };
  }
  ...
  // Accumulate cost (kWh delta \u00d7 current price) into day/month/year buckets for a lifetime-counting energy sensor
  _trackEnergyCost(key, currentEnergyKwh, currentPrice) {
    if (currentEnergyKwh === null || currentPrice === null) return { day: null, month: null, year: null };
    const now = new Date();
    const yearKey = String(now.getFullYear());
    const monthKey = yearKey + '-' + String(now.getMonth()+1).padStart(2,'0');
    const dayKey = monthKey + '-' + String(now.getDate()).padStart(2,'0');
    const storeKey = 'grankulle_energycost_' + key + '_v1';
    let s;
    try { s = JSON.parse(localStorage.getItem(storeKey) || 'null'); } catch(e) { s = null; }
    if (!s) {
      s = { lastEnergy: currentEnergyKwh, day: dayKey, dayTotal: 0, month: monthKey, monthTotal: 0, year: yearKey, yearTotal: 0 };
    } else {
      const delta = Math.max(0, currentEnergyKwh - s.lastEnergy);
      const costDelta = delta * currentPrice;
      if (s.day !== dayKey) { s.day = dayKey; s.dayTotal = 0; }
      if (s.month !== monthKey) { s.month = monthKey; s.monthTotal = 0; }
      if (s.year !== yearKey) { s.year = yearKey; s.yearTotal = 0; }
      s.dayTotal += costDelta;
      s.monthTotal += costDelta;
      s.yearTotal += costDelta;
      s.lastEnergy = currentEnergyKwh;
    }
    try { localStorage.setItem(storeKey, JSON.stringify(s)); } catch(e) {}
    return { day: s.dayTotal, month: s.monthTotal, year: s.yearTotal };
  }

  // Track today's kWh split by which charging mode was active when each increment happened
  _trackEnergyBySource(key, currentEnergyKwh, currentMode) {
    if (currentEnergyKwh === null) return null;
    const now = new Date();
    const dayKey = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0') + '-' + String(now.getDate()).padStart(2,'0');
    const storeKey = 'grankulle_energysource_' + key + '_v1';
    let s;
    try { s = JSON.parse(localStorage.getItem(storeKey) || 'null'); } catch(e) { s = null; }
    if (!s || s.day !== dayKey) {
      s = { day: dayKey, lastEnergy: currentEnergyKwh, byMode: {} };
    } else {
      const delta = Math.max(0, currentEnergyKwh - s.lastEnergy);
      if (delta > 0 && currentMode) {
        s.byMode[currentMode] = (s.byMode[currentMode] || 0) + delta;
      }
      s.lastEnergy = currentEnergyKwh;
    }
    try { localStorage.setItem(storeKey, JSON.stringify(s)); } catch(e) {}
  ...
  // Integrate a live power reading (W) into a running daily kWh total via Riemann sum,
  // using the real elapsed time since the last card update. Resets at midnight.
  _integrateDailyEnergy(key, powerWatts) {
    if (powerWatts === null) powerWatts = 0;
    const now = Date.now();
    const dayKey = new Date().toDateString();
    const storeKey = 'grankulle_daily_energy_' + key + '_v1';
    let s;
    try { s = JSON.parse(localStorage.getItem(storeKey) || 'null'); } catch(e) { s = null; }
    if (!s || s.day !== dayKey) {
      s = { day: dayKey, kwh: 0, lastTs: now };
    } else {
      const hoursElapsed = (now - s.lastTs) / 3600000;
      if (hoursElapsed > 0 && hoursElapsed <= 6) { // guard against huge gaps (page closed overnight etc.)
        s.kwh += (powerWatts / 1000) * hoursElapsed;
      }
      s.lastTs = now;
    }
    try { localStorage.setItem(storeKey, JSON.stringify(s)); } catch(e) {}
  ...
  _buildDom() {
    ...
    this.shadowRoot.addEventListener('click', (ev) => {
      ...
      if (action === 'sp-playpause') {
        const id = this._e('spotify_media_player');
        if (id && this._hass) {
          const st = this._hass.states[id];
          const svc = st && st.state === 'playing' ? 'media_pause' : 'media_play';
          this._hass.callService('media_player', svc, { entity_id: id });
        }
      }
      if (action === 'sp-next') this._call('media_player', 'media_next_track', 'spotify_media_player');
      if (action === 'sp-prev') this._call('media_player', 'media_previous_track', 'spotify_media_player');
      if (action === 'ev1f-mode') {
        const mode = btn.dataset.mode;
        const id = this._e('ev_1fas_mode');
        if (id && this._hass) this._hass.callService('input_select', 'select_option', { entity_id: id, option: mode });
      }
    });
  ...
  _update() {
    ...
    const priceFmt = this._fmtPrice(price);
    this.shadowRoot.getElementById('c-tib-price').innerHTML = priceFmt.text + '<small>' + priceFmt.unit + '</small>';
    ...
    if (ev1fasState) {
      ...
      this._set('c-ev1f-sol', attrs.sol_overskott_w + ' W');
      const solEl = this.shadowRoot.getElementById('c-ev1f-sol');
      if (solEl) {
        solEl.style.color = attrs.sol_overskott_ok ? 'var(--battery)' : 'var(--text-dim)';
        solEl.style.fontWeight = attrs.sol_overskott_ok ? '700' : '500';
      }
      this._set('c-ev1f-pris', attrs.billig_el_ok ? 'Ja' : 'Nej');
      this._set('c-ev1f-batt', attrs.batteri_over_80_ok ? 'Ja' : 'Nej');
      const ev1fPriceFmt = this._fmtPrice(this._num('tibber_price',3));
      this._set('c-ev1f-price', ev1fPriceFmt.text + ' ' + ev1fPriceFmt.unit);
      this._set('c-ev1f-overload', attrs.fas3_overlast_ok ? 'Ja \u2014 laddning paus' : 'Nej');
      const overheatVal = this._val('shelly2_overheat');
      const overheatEl = this.shadowRoot.getElementById('c-ev1f-overheat');
      this._set('c-ev1f-overheat', overheatVal === 'on' ? 'Ja \u2014 laddning blockerad' : 'Nej');
      if (overheatEl) overheatEl.style.color = overheatVal === 'on' ? '#e2634f' : '';
      if (overheatEl) overheatEl.style.fontWeight = overheatVal === 'on' ? '700' : '500';
      this._set('c-ev1f-plug-power', this._fmt(this._num('shelly2_power',0),0,' W'));
      this._set('c-ev1f-plug-temp', this._fmt(this._num('shelly2_temp',1),1,' \u00b0C'));
      const currentMode = this._val('ev_1fas_mode');
      const modeDisplayNames = { endast_sol:'\u2600\ufe0f Endast sol', sol_plus_lite:'\u2600\ufe0f Sol + lite el', billig_el:'\ud83d\udcb0 Billig el', ladda_direkt:'\u26a1 Ladda direkt' };
      this._set('c-ev1f-mode-display', 'L\u00e4ge: ' + (modeDisplayNames[currentMode] || currentMode || '\u2013 (ingen entitet kopplad)'));
      ['endast_sol','sol_plus_lite','billig_el','ladda_direkt'].forEach(m => {
        const btn = this.shadowRoot.getElementById('ev1f-mode-' + m + '-btn');
        if (btn) btn.classList.toggle('active', currentMode === m);
      });

      const plugEnergy = this._num('shelly2_energy', 3);
      const plugPrice = this._num('tibber_price', 3);
      const plugCost = this._trackEnergyCost('shelly2', plugEnergy, plugPrice);
      this._set('c-ev1f-cost-day', this._fmt(plugCost.day, 2, ' SEK'));
      this._set('c-ev1f-cost-month', this._fmt(plugCost.month, 2, ' SEK'));
      this._set('c-ev1f-cost-year', this._fmt(plugCost.year, 2, ' SEK'));

      const modeNames = { endast_sol:'Sol', sol_plus_lite:'Sol+lite el', billig_el:'Billig el', ladda_direkt:'Direkt' };
      const bySource = this._trackEnergyBySource('shelly2', plugEnergy, currentMode);
      if (bySource) {
        const parts = Object.keys(bySource).map(k => (modeNames[k] || k) + ': ' + bySource[k].toFixed(2) + ' kWh');
        this._set('c-ev1f-source-today', parts.length ? parts.join(' \u00b7 ') : '\u2013');
      }
      if (bySource) {
        const totalPlugKwh = Object.values(bySource).reduce((a,b)=>a+b, 0);
        this._set('c-eff-plug', totalPlugKwh.toFixed(2) + ' kWh');
      }
    }

    // Spotify
    const spState = this._st('spotify_media_player');
    if (spState) {
      const isPlaying = spState.state === 'playing';
      const isPaused = spState.state === 'paused';
      const stateNames = { playing:'Spelar', paused:'Pausad', idle:'Inaktiv', off:'Av' };
      this._set('c-sp-chip', stateNames[spState.state] || spState.state || '\u2013');
      const title = spState.attributes.media_title;
      const artist = spState.attributes.media_artist;
      this._set('c-sp-title', title || (isPlaying || isPaused ? '\u2013' : 'Inget spelas'));
      this._set('c-sp-artist', artist || '');
      const ppBtn = this.shadowRoot.getElementById('sp-playpause-btn');
      if (ppBtn) ppBtn.classList.toggle('active', isPlaying);
    }
  ...
  _updateFlow(soc, battPower, battState, froPower) {
    ...
    const battAmpText = this._num('battery_current',1) !== null ? ' \u00b7 ' + this._num('battery_current',1) + ' A' : '';
    this._set('fv-battery', (soc===null?'\u2013':soc) + '% SoC \u00b7 ' + this._fmt(battPower/1000, 2, ' kW') + battAmpText);
    ...
    // Hemmets effektivitet idag (Riemann-summering av effekt \u2192 kWh)
    const solarKwhToday = this._integrateDailyEnergy('solar', froPower);
    const battDischargeW = (battState === 'discharging' && battPower !== null) ? Math.abs(battPower) : 0;
    const battChargeW = (battState === 'charging' && battPower !== null) ? Math.abs(battPower) : 0;
    const battDischargeKwhToday = this._integrateDailyEnergy('batt_discharge', battDischargeW);
    const battChargeKwhToday = this._integrateDailyEnergy('batt_charge', battChargeW);
    const consumptionKwhToday = this._integrateDailyEnergy('house_consumption', houseOnlyPower);
    const gridImportW = gridSum > 0 ? gridSum : 0;
    const gridImportKwhToday = this._integrateDailyEnergy('grid_import', gridImportW);
    const selfSuffPct = consumptionKwhToday > 0.01
      ? Math.max(0, Math.min(100, (1 - (gridImportKwhToday / consumptionKwhToday)) * 100))
      : null;
    this._set('c-eff-percent', selfSuffPct === null ? '\u2013' : Math.round(selfSuffPct));
    const effPercentEl = this.shadowRoot.getElementById('c-eff-percent');
    if (effPercentEl) effPercentEl.innerHTML = (selfSuffPct === null ? '\u2013' : Math.round(selfSuffPct)) + '<small>% sj\u00e4lvf\u00f6rs\u00f6rjt</small>';
    this._set('c-eff-solar', solarKwhToday.toFixed(2) + ' kWh');
    this._set('c-eff-battdis', battDischargeKwhToday.toFixed(2) + ' kWh');
    this._set('c-eff-battchg', battChargeKwhToday.toFixed(2) + ' kWh');
    this._set('c-eff-consumption', consumptionKwhToday.toFixed(2) + ' kWh');

    const zaptecEnergyVal = this._num('zaptec_energy', 3);
    this._set('c-eff-zaptec', zaptecEnergyVal === null ? '\u2013 (ingen sensor kopplad)' : zaptecEnergyVal.toFixed(2) + ' kWh');
    ...
    const froAmpTotal = ['fronius_current_l1','fronius_current_l2','fronius_current_l3'].map(k=>this._num(k,1)).filter(v=>v!==null);
    const froAmpText = froAmpTotal.length ? ' \u00b7 ' + froAmpTotal.reduce((a,b)=>a+b,0).toFixed(1) + ' A' : '';
    this._set('fv-fronius', froPower === null ? '\u2013' : this._fmt(froPower/1000, 2, ' kW') + froAmpText);
    ...
    const zapAmpTotal = ['zaptec_current_l1','zaptec_current_l2','zaptec_current_l3'].map(k=>this._num(k,1)).filter(v=>v!==null);
    const zapAmpText = zapAmpTotal.length ? ' \u00b7 ' + zapAmpTotal.reduce((a,b)=>a+b,0).toFixed(1) + ' A' : '';
    const zapEvText = zapCharging === 'on'
      ? ('Laddar' + (zapPowerVal !== null ? ' \u00b7 ' + (zapPowerVal/1000).toFixed(1) + ' kW' : '') + zapAmpText)
