...
class GrankulleEnergyCard extends HTMLElement {
  ...
  // Tibber's daily reward sensor resets each midnight, so we accumulate a real
  // month-to-date total client-side in localStorage (browser-local, per device).
  _monthlyRewardText(todayReward) {
    if (todayReward === null) return '\u2013';
    const now = new Date();
    const monthKey = now.getFullYear() + '-' + String(now.getMonth()+1).padStart(2,'0');
    const dayKey = monthKey + '-' + String(now.getDate()).padStart(2,'0');
    const storeKey = 'grankulle_monthly_reward_v1';
    let stored;
    try { stored = JSON.parse(localStorage.getItem(storeKey) || 'null'); } catch(e) { stored = null; }
    if (!stored || stored.month !== monthKey) {
      stored = { month: monthKey, total: 0, lastDay: dayKey, lastDayValue: todayReward };
    } else if (stored.lastDay !== dayKey) {
      stored.total += stored.lastDayValue;
      stored.lastDay = dayKey;
      stored.lastDayValue = todayReward;
    } else {
      stored.lastDayValue = todayReward;
    }
    try { localStorage.setItem(storeKey, JSON.stringify(stored)); } catch(e) {}
    const monthTotal = stored.total + stored.lastDayValue;
    return monthTotal.toFixed(2) + ' SEK denna m\u00e5nad';
  }
  ...
  _update() {
    ...
    this._set('c-zap-current', zapCurrent === null ? '\u2013' : zapCurrent + ' A');
    ...
    this._set('c-tib-reward', this._monthlyRewardText(reward));
  ...
  _updateFlow(soc, battPower, battState, froPower) {
    ...
    const houseAmpTotal = ['house_current_l1','house_current_l2','house_current_l3'].map(k=>this._num(k,1)).filter(v=>v!==null);
    const gridAmpText = gridAmpTotal.length ? ' \u00b7 ' + gridAmpTotal.reduce((a,b)=>a+b,0).toFixed(1) + ' A' : '';
    const houseAmpText = houseAmpTotal.length ? ' \u00b7 ' + houseAmpTotal.reduce((a,b)=>a+b,0).toFixed(1) + ' A' : '';
    this._set('fv-grid', this._fmt(gridSum/1000, 2, ' kW') + (gridSum >= 0 ? ' in' : ' ut') + gridAmpText);
    this._set('fv-house', this._fmt(houseSum/1000, 2, ' kW') + houseAmpText);
    ...
    const setEdge = (pathId, dotId, motionId, dValue, isActive) => {
      ...
      const motion = this.shadowRoot.getElementById(motionId);
      if (p && dValue) p.setAttribute('d', dValue);
      if (motion && dValue) motion.setAttribute('path', dValue);
    ...
    const battChargeD = 'M 335 108 C 335 92, 335 78, 335 58';
    const battDischargeD = 'M 335 58 C 335 78, 335 92, 335 108';
    ...
    const gridInImportD = 'M 170 226 C 260 226, 300 190, 320 174';
    const gridInExportD = 'M 320 174 C 300 190, 260 226, 170 226';
    const gridOutImportD = 'M 470 138 C 560 138, 580 128, 620 116';
    const gridOutExportD = 'M 620 116 C 580 128, 560 138, 470 138';
