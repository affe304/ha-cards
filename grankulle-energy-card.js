...
class GrankulleEnergyCard extends HTMLElement {
  ...
  _buildDom() {
    ...
    this.shadowRoot.addEventListener('click', (ev) => {
      ...
      if (action === 'tv-playpause') this._call('media_player', 'media_play_pause', 'tv_media_player');
      if (action === 'tv-next') this._call('media_player', 'media_next_track', 'tv_media_player');
      if (action === 'tv-prev') this._call('media_player', 'media_previous_track', 'tv_media_player');
      if (action === 'net-pause') {
        const client = btn.dataset.client;
        const minutes = parseInt(btn.dataset.minutes, 10);
        if (this._hass && this._e('adguard_status')) {
          this._hass.callService('script', 'adguard_pause_device', { client, minutes });
        }
      }
      if (action === 'net-unpause') {
        const client = btn.dataset.client;
        if (this._hass && this._e('adguard_status')) {
          this._hass.callService('script', 'adguard_unpause_device', { client });
        }
      }
    });
  ...
  _update() {
    ...
    // N\u00e4tverkspaus (AdGuard)
    const devices = this._config.network_devices || [];
    const adguardStatusId = this._e('adguard_status');
    const adguardState = adguardStatusId ? this._st('adguard_status') : null;
    const disallowed = adguardState ? (adguardState.attributes.disallowed_clients || []) : null;
    const listEl = this.shadowRoot.getElementById('net-device-list');
    if (listEl && devices.length) {
      listEl.innerHTML = devices.map(d => {
        const isPaused = disallowed !== null ? disallowed.includes(d.client) : null;
        const statusText = isPaused === null ? '\u2013' : (isPaused ? 'Pausad' : 'Aktiv');
        const statusColor = isPaused ? 'var(--price-high)' : 'var(--battery)';
        return '<div class="device">'
          + '<div class="device-left"><div class="device-icon">\ud83d\udcf6</div><div>'
          + '<div class="device-name">' + d.name + '</div>'
          + '<div class="device-room" style="color:' + statusColor + '">' + statusText + '</div>'
          + '</div></div>'
          + '<div class="device-right" style="gap:8px;">'
          + '<button class="valve-btn off-btn" style="flex:none; min-width:auto; padding:8px 12px; font-size:11px;" data-action="net-unpause" data-client="' + d.client + '">Aktivera</button>'
          + '<button class="valve-btn" style="flex:none; min-width:auto; padding:8px 12px; font-size:11px;" data-action="net-pause" data-client="' + d.client + '" data-minutes="30">30 min</button>'
          + '<button class="valve-btn" style="flex:none; min-width:auto; padding:8px 12px; font-size:11px;" data-action="net-pause" data-client="' + d.client + '" data-minutes="60">1 tim</button>'
          + '</div></div>';
      }).join('');
      this._set('c-net-count', devices.length + ' enheter');
    } else if (listEl) {
      listEl.innerHTML = '<div class="sub-line">Inga enheter konfigurerade \u2014 l\u00e4gg till <code>network_devices</code> i kortets inst\u00e4llningar.</div>';
      this._set('c-net-count', '0 enheter');
    }

    // The Frame / Spotify
    const tvState = this._st('tv_media_player');
    if (tvState) {
      const isPlaying = tvState.state === 'playing';
      const isPaused = tvState.state === 'paused';
      const stateNames = { playing:'Spelar', paused:'Pausad', idle:'Inaktiv', off:'Av', on:'P\u00e5' };
      this._set('c-tv-chip', stateNames[tvState.state] || tvState.state || '\u2013');
      const title = tvState.attributes.media_title;
      const artist = tvState.attributes.media_artist;
      this._set('c-tv-title', title || (isPlaying || isPaused ? '\u2013' : 'Inget spelas'));
      this._set('c-tv-artist', artist || '');
      const ppBtn = this.shadowRoot.getElementById('tv-playpause-btn');
