...
class GrankulleEnergyCardV3 extends HTMLElement {
  ...
  _update() {
    ...
    const tibI1 = this._num('tibber_current_l1',1), tibI2 = this._num('tibber_current_l2',1), tibI3 = this._num('tibber_current_l3',1);
    this._set('c-tib-current', [tibI1,tibI2,tibI3].every(v=>v!==null) ? tibI1+' / '+tibI2+' / '+tibI3+' A' : '\u2013');
    ...
    if (tadoState) {
      ...
      this._set('c-tado-target', isNaN(targetTemp) ? '\u2013' : Math.round(targetTemp).toString());
    ...
    // "Just nu" quick panel (top-left of the scene) — same figures as the flow diagram's Hus/Fronius nodes
    const zapPowerQP = this._num('zaptec_power', 0) || 0;
    const houseSumQP = ['house_power_l1','house_power_l2','house_power_l3']
      .map(k => this._num(k, 0)).filter(v => v !== null).reduce((a,b) => a+b, 0);
    const houseOnlyQP = Math.max(0, (houseSumQP - zapPowerQP) / 1000);
    const solarQP = (froPower || 0) / 1000;
    const battPowerQP = (battPower || 0) / 1000;
    const gridSumQP = ['grid_power_l1','grid_power_l2','grid_power_l3']
      .map(k => this._num(k, 0)).filter(v => v !== null).reduce((a,b) => a+b, 0) / 1000;
    const selfSuffQP = houseOnlyQP > 0
      ? Math.min(100, Math.max(0, Math.round(((solarQP + Math.max(0, -battPowerQP)) / houseOnlyQP) * 100)))
      : 100;
    this._set('qp-selfsuff', selfSuffQP + '%');
    this._set('qp-solar', solarQP.toFixed(2) + ' kW');
    this._set('qp-house', houseOnlyQP.toFixed(2) + ' kW');
    this._set('qp-batt', (soc === null ? '\u2013' : Math.round(soc)) + '% \u00b7 ' + battPowerQP.toFixed(2) + ' kW');
    this._set('qp-grid', gridSumQP.toFixed(2) + ' kW');

    // Dagens energi (Tibber accumulated today)
    this._set('c-day-solar', production === null ? '\u2013' : production.toFixed(1) + ' kWh');
    this._set('c-day-house', consumption === null ? '\u2013' : consumption.toFixed(1) + ' kWh');
  }
  ...
  _updateFlow(soc, battPower, battState, froPower) {
    ...
    const battChargeD = 'M 350 160 C 350 147, 350 133, 350 120';
    const battDischargeD = 'M 350 120 C 350 133, 350 147, 350 160';
    ...
    const gridInImportD = 'M 350 310 C 350 297, 350 283, 350 270';
    const gridInExportD = 'M 350 270 C 350 283, 350 297, 350 310';
    const gridOutImportD = 'M 470 185 C 600 130, 680 80, 720 65';
  ...
  // ---------- day / night cycle, driven by sun.sun elevation (falls back to local clock hour) ----------
  _applyDayNight() {
    const sceneCard = this.shadowRoot.getElementById('sceneCard');
    if (!sceneCard) return;
    const sunId = this._e('sun') || 'sun.sun';
    const sunState = this._hass && this._hass.states[sunId];
    let elevation = null;
    if (sunState && sunState.attributes && typeof sunState.attributes.elevation === 'number') {
      elevation = sunState.attributes.elevation;
    }
    let phase;
    if (elevation !== null) {
      if (elevation > 4) phase = 'day';
      else if (elevation > -4) phase = 'dusk';
      else phase = 'night';
    } else {
      const h = new Date().getHours();
      if (h >= 7 && h < 19) phase = 'day';
      else if ((h >= 19 && h < 21) || (h >= 5 && h < 7)) phase = 'dusk';
      else phase = 'night';
    }
    sceneCard.setAttribute('data-daytime', phase);
  }
  ...
  _applySeasonAndWeather() {
    ...
    fetch(forecastUrl)
      ...
      .then(data => {
        ...
        const arrow = this.shadowRoot.getElementById('c-wind-arrow');
        if (arrow && typeof dir === 'number') arrow.setAttribute('transform', 'rotate(' + dir + ' 30 30)');
        this._set('c-wind-speed', '');
        const speedEl = this.shadowRoot.getElementById('c-wind-speed');
        if (speedEl) speedEl.innerHTML = (typeof speed === 'number' ? speed.toFixed(1) : '\u2013') + '<small>m/s</small>';
        this._set('c-wind-dir', (typeof dir === 'number' ? Math.round(dir) : '\u2013') + '\u00b0');

        // 5-day forecast row
        const forecastEl = this.shadowRoot.getElementById('c-forecast');
        if (forecastEl && data.daily) {
          const dayNames = ['S\u00f6n','M\u00e5n','Tis','Ons','Tor','Fre','L\u00f6r'];
          const iconFor = { clear:'\u2600\ufe0f', cloudy:'\u26c5', rain:'\ud83c\udf27\ufe0f', snow:'\u2744\ufe0f' };
          forecastEl.innerHTML = data.daily.time.map((t, i) => {
            const d = new Date(t);
            const label = i === 0 ? 'Idag' : dayNames[d.getDay()];
            const wcode = classifyWeatherCode(data.daily.weather_code[i]);
            return '<div class="fday">' +
              '<div class="fday-lbl">' + label + '</div>' +
              '<div class="fday-icon">' + (iconFor[wcode] || '\u2013') + '</div>' +
              '<div class="fday-hi">' + Math.round(data.daily.temperature_2m_max[i]) + '\u00b0</div>' +
              '<div class="fday-lo">' + Math.round(data.daily.temperature_2m_min[i]) + '\u00b0</div>' +
              '<div class="fday-rain">' + data.daily.precipitation_sum[i].toFixed(1) + ' mm</div>' +
              '</div>';
          }).join('');
        }
      })
...
customElements.define('grankulle-energy-card-v3', GrankulleEnergyCardV3);

// Register with the Lovelace card picker (optional but nice-to-have)
window.customCards = window.customCards || [];
window.customCards.push({
  type: 'grankulle-energy-card-v3',
  name: 'Grankullevägen 19 — Energy Card v3',
  description: 'Live illustrerad SCADA-scen med "Just nu", väderprognos och vindkompass — byggd från grunden, exakt positionerad.'
});
